...
options = Options.new({ "target"    => "k1",
			...
			"toolchain"  => { "type"     => "keywords",
					  "keywords" => [:default, :bare, :rtems, :linux],
					  "default"  => "default",
					  "help"     => "Enable to build only utils i.e. for bare toolchain"
					},

			"build_type"=> {"type" => "keywords", "keywords" => [:Debug, :Release], "default" => "Debug", "help" => "Build type." },
		      })
...
gbu_processor_prefix = "#{processor_path}/k1-family/BE/GBU/k1b/bfd/" # [PG] Path should be march dependent
...
b.target("build") do
  ...
  when "k1"
	...
	flag_opt = " -mos=#{os_flav} #{multi_opts}"
	...
	# cmd = "LIBDIR=#{libdir} " +
	#       "PATH=#{toolroot}/bin:#{install_prefix}/bin:$PATH " +
	#       "CFLAGS='#{flag_opt} #{extra_flags} #{autotraces_flags} -I#{gbu_processor_prefix}' " +
	#       "LDFLAGS='#{flag_opt} #{extra_flags}' " +
	#       "#{mppadl_path}/configure " +
	#       "--host=#{arch} " +
	#       "--bindir=#{build_dir}/install_bin " +
	#       "--libdir=#{libdir} " +
...
b.target("package") do
  ...
  if (distrib_info.name != "MinGW")
    ...
    pinfo = b.package_info(mppadl_name, release_info,
