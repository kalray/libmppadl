...
extern const char const_char_s_mdo[];
...
int main(int argc, char *argv[])
{
  ...
  if (mppa_dl_load_buffer(handle, const_char_s_mdo) == -1) {
  ...
  if ((f_ptr = mppa_dl_sym(handle, "mppa_dl_main")) == NULL) {
    fprintf(stderr, "No mppa_dl_main found in const_char_s module\n");
    goto fail;
  } else {
    printf("Found mppa_dl_main in const_char_s module at {{%p}}\n",  f_ptr);
